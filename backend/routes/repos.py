...
def _delete_repo_records(conn: sqlite3.Connection, repo_id: int) -> None:
    ...
    conn.execute("DELETE FROM web_ui_prompts WHERE repo_id = ?", (repo_id,))
    conn.execute("DELETE FROM web_ui_repo_profiles WHERE repo_id = ?", (repo_id,))
    conn.execute("DELETE FROM skill_repo_profiles WHERE repo_id = ?", (repo_id,))
